...
int main(int argc, char** argv)
{
   ...
   private_nh.param("file_directory", file_directory, std::string("/home/dikai/下载/"));
   ROS_INFO("*** file_directory = %s ***\n", file_directory.c_str());


   private_nh.param("file_name", file_name, std::string("new3Dmap"));
   ROS_INFO("*** file_name = %s ***\n", file_name.c_str());

   pcd_file = file_directory + file_name + pcd_format;
   ROS_INFO("*** pcd_file = %s ***\n", pcd_file.c_str());

   private_nh.param("thre_z_min", thre_z_min, 0.7);
   private_nh.param("thre_z_max", thre_z_max, 1.2);
   ...
   private_nh.param("thre_radius", thre_radius, 1.2);
   private_nh.param("map_resolution", map_resolution, 0.02);
